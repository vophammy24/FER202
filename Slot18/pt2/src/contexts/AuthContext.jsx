...
export const AuthProvider = ({ children }) => {
  ...
  const login = async ({identifier, password}) => {
    ...
    const isEmail = validateEmail(identifier) === null;
    ...
    try {
      ...
      if (user){
        ...
        if(user.role !== 'admin'){
          const errorMessage = 'Access denied. Only admin users can login.';
          dispatch({
            type: 'LOGIN_FAILURE',
        ...
        if(user.status !== 'active'){
          const errorMessage = 'Account is locked. Please contact administrator.';
          dispatch({
            type: 'LOGIN_FAILURE',
            payload: errorMessage
          })
